#!/bin/sh
# Apply or check a layout document (ignite.layout/1).
...
usage() {
	cat <<'EOF'
usage:
  layout.sh analyze [--layout FILE] [--kind KIND] [--dest DIR] [workspace]
  layout.sh init    [--layout FILE] [--kind KIND] [--dest DIR] [--parent-kind KIND] [workspace]

Defaults come from <workspace>/ignite.toml [layout] (file, kind).
...
while [ "$#" -gt 0 ]; do
	...
	--)
		...
		;;
	-*)
		echo "ignite layout: unknown option $1" >&2
		usage >&2
		exit 1
		;;
	*)
		if [ -n "$WORKSPACE_ARG" ]; then
			echo "ignite layout: unexpected argument $1" >&2
			exit 1
		fi
		WORKSPACE_ARG=$1
		shift
...
analyze | init) ;;
*)
	echo "ignite layout: unknown command $CMD" >&2
	usage >&2
	exit 1
	;;
esac
...
if [ -z "$LAYOUT_FILE" ] || [ -z "$LAYOUT_KIND" ]; then
	echo "ignite layout: need --layout and --kind, or ignite.toml [layout]" >&2
	exit 1
fi
...
if [ -n "$DEST_ARG" ]; then
	...
	echo "ignite layout: need --dest or a workspace" >&2
	exit 1
fi
...
if [ "$CMD" = init ] && [ -n "$WORKSPACE_ROOT" ] && [ "$DEST" != "$WORKSPACE_ROOT" ]; then
	...
	if [ -n "$_parent" ] && [ "$_parent" != "$LAYOUT_KIND" ]; then
		if ! layout_contains_kind "$_parent" "$LAYOUT_KIND"; then
			echo "ignite layout: kind '$LAYOUT_KIND' is not in contains of '$_parent'" >&2
			exit 1
		fi
	fi
fi
